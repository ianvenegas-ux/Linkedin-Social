...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    try {
      ...
      const publishedUpdate = buildPublishedPostUpdate(
        post,
        actorId,
        postUrn,
        publishedUrl,
      ...
      const { error: markPublishedError } = await admin
        .from("social_posts")
        .update(publishedUpdate)
        .eq("id", post.id);
      if (markPublishedError) {
        throw new Error("Could not mark social post as published: " + markPublishedError.message);
      }
